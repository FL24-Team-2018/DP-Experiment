...
xlabel('\sigma_{est}/um'), ylabel('I_{rec,min}');

subplot(nrows,1,3)
plot(sigma_est_um_array,xi_um_hor,'-xb');
xlabel('\sigma_{est}/um'), ylabel('\xi / um');
...
xlabel('\sigma_{est}/um'), ylabel('\zeta');

subplot(nrows,1,6)
plot(sigma_est_um_array,D,'-xb');
xlabel('\sigma_{est}/um'), ylabel('histogram dist.\newlineI_{pc,det} vs. I_{pc,rec}');

subplot(nrows,1,7)
plot(sigma_est_um_array,corr_I_pixis_I_pc_rec,'-xb');
xlabel('\sigma_{est}/um'), ylabel('histogramm corr.\newline I_{pc,det} vs. I_{pc,rec}');
...
figure('rend', 'painters','pos', [1010 510 1000 1000]);
...
%%
figure('rend', 'painters','pos', [1010 10 700 2100]);
...
figure('rend', 'painters','pos', [1010 100 2600 700]);
...
subplot(1,3,2);
imagesc(X1_axis*R_1,Y1_axis*R_1,I_rec(:,:,i)); colormap(gca,jet);
title(strcat('recovered fully coherent intensity I_{rec} at CCD'));colorbar; axis equal;
caxis([0 1]);
xlabel('x/um'), ylabel('y/um');
axis equal tight;

subplot(1,3,3);
imagesc(X1_axis*R_1,Y1_axis*R_1,F_gamma_rec(:,:,i)); colormap(gca,jet); title(strcat('recovered F(\gamma)_{rec} at CCD, \sigma_{rec,Gauss}=',num2str(round(sigma_F_gamma_rec_hor_um(i),1)),'um'));colorbar; 
%axis equal;
xlabel('x/um'), ylabel('y/um');
axis equal tight;
xlim([-100 100]);
...
saveas(gcf,fullfile(cd,folder,strcat('partiallycoherentimageandFgamma_',num2str(sigma_est_um_array(i)),'um')),'epsc');
saveas(gcf,fullfile(cd,folder,strcat('partiallycoherentimageandFgamma_',num2str(sigma_est_um_array(i)),'um')),'png');

%% lineouts of partially coherent measurement and fully coherent reconstruction
figure('rend', 'painters','pos', [100 100 1500 1100],'visible','on');
deltay = 25;
...
title(strcat('\lambda = ',num2str(wavelength_nm),'nm', ', separation d=',num2str(d_um),'um, \sigma_B=',num2str(sigma_B_um),'um, ', '\newline coherence length \xi=',num2str(round(xi_um_hor(i),1)),'um, \xi/\sigma_B=' ,num2str(round(xi_um_hor(i)/sigma_B_um,2)),', \zeta=',num2str(round(zeta(i),2))));
xlim([-6500,6500])
xlabel('detector position / um')
ylabel('Intensity / a.u.')
saveas(gcf,fullfile(cd,folder,strcat('profiles_sigma_est_um_opt_',num2str(sigma_est_um_array(i)),'um')),'epsc');
saveas(gcf,fullfile(cd,folder,strcat('profiles_sigma_est_um_opt_',num2str(sigma_est_um_array(i)),'um')),'png');

for i=1:length(sigma_est_um_array)
    figure('rend', 'painters','pos', [100 100 1500 1100],'visible','off');
    deltay = 25;
    I_pixis_profile = norm_function(mean(I_pixis(idx_row-deltay:idx_row+deltay,:)));
    min(I_pixis_profile(round(n/2)-200:round(n/2)+200))
    plot(X1_axis*R_1,I_pixis_profile,'-b', 'LineWidth',3);
    ...
    saveas(gcf,fullfile(cd,folder,strcat('profiles_sigma_est_um_',num2str(sigma_est_um_array(i)),'um')),'epsc');
    saveas(gcf,fullfile(cd,folder,strcat('profiles_sigma_est_um_',num2str(sigma_est_um_array(i)),'um')),'png');
end

%% pinhole image
figure('rend', 'painters','pos', [510 510 1000 1000]);
imagesc(pinholes_image_norm); colormap(gca,jet);
saveas(gcf,fullfile(cd,folder,'pinhole_image'),'epsc');
saveas(gcf,fullfile(cd,folder,'pinhole_image'),'png');
